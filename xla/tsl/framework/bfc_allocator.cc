...
bool BFCAllocator::Extend(size_t alignment, size_t rounded_bytes) {
  ...
  if (mem_addr == nullptr) {
    ...
    while (mem_addr == nullptr) {
      ...
      // RoundedBytes rounds up to kMinAllocationSize, so for small sizes
      // (bytes <= 10 * kMinAllocationSize) the backpedal can round back up to
      // the same value. Force strict progress so a persistently failing
      // sub-allocator cannot make this loop spin forever.
