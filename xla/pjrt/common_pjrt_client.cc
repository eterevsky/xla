...
absl::StatusOr<PjRtDeviceEventRef> CommonPjRtClient::LinearizeIntoImpl(
  ...
  if (raw_buffer->GetHostPointer() == nullptr &&
    ...
    if (host_buffer_semantics ==
      ...
      // order is unspecified, and on Windows clang evaluates the lambda
      // argument (which moves `linearized`) before the dependency-list
      // argument, so `linearized.CopyRCRef()` inline in the argument list
      // would produce a null reference and crash in RunWhenReady.
